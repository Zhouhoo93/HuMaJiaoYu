...
-(instancetype)initWithFrame:(CGRect)frame andLineChartType:(JHLineChartType)lineChartType{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _pointColorArr = @[[UIColor clearColor]];
...
//            _perXLen = (_xLength-0)/;
        ...
        {
            _perXLen = (_xLength/2-0)/[_xLineDataArr[0] count];
            _perYlen = (_yLength-kXandYSpaceForSuperView)/_yLineDataArr.count;
        }
            break;
...
- (void)configChartXAndYLength{
    _xLength = CGRectGetWidth(self.frame)-self.contentInsets.left-self.contentInsets.left-10;
...
- (void)updateYScale{
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstAndFouthQuardrant:{
            ...
            //            _yLineDataArrRight = [minArr copy];
...
- (void)configChartOrigin{
    ...
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstQuardrant:
        {
            ...
            self.chartOriginRight = CGPointMake(self.frame.size.width, self.frame.size.height-self.contentInsets.bottom);
        }
            break;
        case JHLineChartQuadrantTypeFirstAndSecondQuardrant:
        {
...
- (void)drawXAndYLineWithContext:(CGContextRef)context{
    ...
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstQuardrant:{
            ...
            if (_showYLine) {
                [self drawLineWithContext:context andStarPoint:self.chartOrigin andEndPoint:P_M(self.chartOrigin.x,self.chartOrigin.y-_yLength+20) andIsDottedLine:NO andColor:self.xAndYLineColor];
                [self drawLineWithContext:context andStarPoint:CGPointMake(self.chartOriginRight.x-self.contentInsets.left, self.chartOriginRight.y) andEndPoint:P_M(self.chartOriginRight.x-self.contentInsets.left,self.chartOriginRight.y-_yLength+20) andIsDottedLine:NO andColor:self.xAndYLineColor];
            }
            
            if (_xLineDataArr.count>0) {
                CGFloat xPace = (_xLength-0)/(_xLineDataArr.count-1);
            ...
            if (_yLineDataArr.count>0) {
                ...
                for (NSInteger i = 0; i<_yLineDataArr.count; i++) {
                    ...
                    if (_isShowLeft) {
                        if (_isKw) {
                            ...
                            CGFloat number = [_yLineDataArr[i] floatValue];
                            [self drawText:[NSString stringWithFormat:@"%.2f",number] andContext:context atPoint:P_M(p.x-18-3, p.y-hei / 2) WithColor:RGBColor(0, 60, 255) andFontSize:self.yDescTextFontSize];
                        }
                ...
                if (_isShowRight) {
                    for (NSInteger i = 0; i<_yLineDataArr.count; i++) {
                        CGPoint p = P_M(self.chartOriginRight.x, self.chartOriginRight.y - (i+1)*yPace);
        ...
        case JHLineChartQuadrantTypeFirstAndSecondQuardrant:{
            ...
            if (_xLineDataArr.count == 2) {
                ...
                CGFloat xPace = (_xLength/2-0)/(rightArr.count-1);
    ...
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstQuardrant:{
            if ([_yLineDataArr[0] floatValue] == 0) {
                ...
                _perValue = _perYlen/[_yLineDataArr[0] floatValue];
            }
...
- (void)drawPathWithDataArr:(NSArray *)dataArr andIndex:(NSInteger )colorIndex{
    ...
    UIColor *color = (_valueLineColorArr[colorIndex]);
...
- (void)drawPoint{
    ...
    for (NSInteger m = 0;m<_drawDataArr.count;m++) {
        ...
        for (NSInteger i = 0; i<arr.count; i++) {
            ...
            UIColor *color = _pointColorArr.count==_drawDataArr.count?(_pointColorArr[m]):([UIColor clearColor]);
